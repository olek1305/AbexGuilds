<template>
    <tr class="text-center">
        ...
        <td :class="colorColumn(props.columnIndex)" class="clickColumn" @click="redirectToEdit(player)">
            <span class="link-text">Edit</span>
        </td>
        <td :class="colorColumn(props.columnIndex)" class="clickColumn" @click="askToDestroy(player)">
...
const colorColumn = (index) => index % 2 === 0
    ? 'dark:bg-cyan-500 dark:text-gray-900 bg-orange-300 text-black'
    : 'dark:bg-sky-700 dark:text-black bg-amber-400 text-black';
...
<style scoped>
.clickColumn:hover {
    @apply border-2 cursor-pointer dark:text-indigo-100 border-white;
}
</style>
